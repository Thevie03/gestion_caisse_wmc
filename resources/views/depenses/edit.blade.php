    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('depenses.update', $depense) }}" class="needs-validation"
                        novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('categorie') is-invalid @enderror" id="categorie"
                                    ...
                                    <option value="Loyer"
                                        {{ old('categorie', $depense->categorie) == 'Loyer' ? 'selected' : '' }}>Loyer
                                    </option>
                                    <option value="Électricité"
                                        {{ old('categorie', $depense->categorie) == 'Électricité' ? 'selected' : '' }}>
                                        Électricité</option>
                                    <option value="Eau"
                                        {{ old('categorie', $depense->categorie) == 'Eau' ? 'selected' : '' }}>Eau
                                    </option>
                                    <option value="Transport"
                                        {{ old('categorie', $depense->categorie) == 'Transport' ? 'selected' : '' }}>
                                        Transport</option>
                                    <option value="Fournitures"
                                        {{ old('categorie', $depense->categorie) == 'Fournitures' ? 'selected' : '' }}>
                                        Fournitures</option>
                                    <option value="Marketing"
                                        {{ old('categorie', $depense->categorie) == 'Marketing' ? 'selected' : '' }}>
                                        Marketing</option>
                                    <option value="Maintenance"
                                        {{ old('categorie', $depense->categorie) == 'Maintenance' ? 'selected' : '' }}>
                                        Maintenance</option>
                                    <option value="Formation"
                                        {{ old('categorie', $depense->categorie) == 'Formation' ? 'selected' : '' }}>
                                        Formation</option>
                                    <option value="Autres"
                                        {{ old('categorie', $depense->categorie) == 'Autres' ? 'selected' : '' }}>
                                        Autres</option>
                                </select>
                                @error('categorie')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0.01"
                                    ...
                                    name="montant" value="{{ old('montant', $depense->montant) }}" placeholder="0.00"
                                    required>
                                @error('montant')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="date" class="form-control @error('date') is-invalid @enderror"
                                    id="date" name="date"
                                    value="{{ old('date', $depense->date_depense->format('Y-m-d')) }}" required>
                                @error('date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select class="form-select @error('boutique_id') is-invalid @enderror" id="boutique_id"
                                    ...
                                    @foreach ($boutiques as $boutique)
                                        <option value="{{ $boutique->id }}"
                                            {{ old('boutique_id', $depense->boutique_id) == $boutique->id ? 'selected' : '' }}>
                                            {{ $boutique->nom }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('justificatif') is-invalid @enderror" id="justificatif" name="justificatif"
                                rows="3" placeholder="Ajoutez des détails ou justifications pour cette dépense...">{{ old('justificatif', $depense->notes) }}</textarea>
                            @error('justificatif')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">
                        ...
                        <div class="alert alert-info">
                            ...
                            <strong>Note :</strong> Cette modification sera enregistrée avec la date et l'heure
                            actuelles.
                        </div>

                        <!-- Boutons d'action -->
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('depenses.show', $depense) }}" class="btn btn-outline-secondary">
